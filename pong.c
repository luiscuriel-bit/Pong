...
void update_pong(struct Pong* pong, double dt)
{
	if (pong->state == PLAY)
	{
		// The AI in this assignment tries to keep the upper border of the ball aligned with the center of the paddles
		// AI_turn will be true when it's player1's turn and false when it's player2's turn
		if (pong->AI_turn == true)
		{
			pong->player2.vy = 0;

			if (pong->ball.y > pong->player1.y + PADDLE_HEIGHT / 2)
			{
				pong->player1.vy = PADDLE_SPEED;
			}
			else if (pong->ball.y < pong->player1.y + PADDLE_HEIGHT / 2)
			{
				pong->player1.vy = -PADDLE_SPEED;
			}
			else
...
void render_pong(struct Pong pong, struct Fonts fonts)
{
	...
	al_draw_text(fonts.score_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2 - 50, TABLE_HEIGHT / 6, ALLEGRO_ALIGN_CENTER, score);
	sprintf(score, "%d", pong.player2_score);
	al_draw_text(fonts.score_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2 + 50, TABLE_HEIGHT / 6, ALLEGRO_ALIGN_CENTER, score);

	if (pong.state == START)
	{
		al_draw_text(fonts.large_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to start");
	}
	else if (pong.state == SERVE)
	{
		al_draw_text(fonts.large_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to serve");
	}
	else if (pong.state == DONE)
	{
		char winner_message[14];
		sprintf(winner_message, "Player %d won!", pong.winning_player);
		al_draw_text(fonts.large_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2, TABLE_HEIGHT / 3, ALLEGRO_ALIGN_CENTER, winner_message);
		al_draw_text(fonts.large_font, al_map_rgb(235, 87, 39), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to restart");
	}
}
